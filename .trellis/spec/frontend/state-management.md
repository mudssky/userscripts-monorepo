...
- **React 展示状态**：使用 `useState` 接收配置快照和状态快照；持久化写入后重新读取校验后的配置，避免 UI 持有未校验对象。
- **React 外部状态**：跨组件或跨模块状态优先用小型 store 暴露 `getStatus()` / `setStatus()` / `subscribe()`，组件通过订阅同步，不为单脚本局部需求引入 Redux、Zustand 等全局状态库。
...
React userscript 中，配置仓库、状态仓库和自动化控制器可以在 `main.tsx` 入口初始化为模块级单例；展示组件通过 props 接收这些依赖，便于测试和未来替换。
...
- 不要让 React state 直接信任 GM storage 或 JSON.parse 结果；进入业务前必须通过配置校验/归一化函数。
