...
在复制模式下遇到虚拟滚动表格时，脚本会自动滚动收集已加载的结果行。为避免长表格复制过慢，默认超过 300 行会弹出确认；点击「确定」继续完整复制，点击「取消」只复制前 300 行。
...
├── copy-mode.ts  # 复制模式：油猴菜单切换与持久化
├── csv-exporter.ts # CSV 导出：捕获 DMS 原生导出内容
